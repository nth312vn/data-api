...
Dynamic SQL is parsed with SQLGlot's Trino dialect and must be exactly one
...
the validator is the only SQL sent to Trino. Query values use typed
...
worker restarts. The table stores original SQL for admin review, canonical SQL
for execution, typed parameter definitions and ownership metadata. There is
no in-memory route registry, PII mapping, `pii_columns`, `lab_test_result`,
versioning or soft delete. DELETE is hard delete.
