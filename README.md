...
## Dynamic API (Sprint 5)

Trusted internal admins/data engineers can register read-only Dynamic Routes
through `/api/v1/dynamic-routes`. Runtime execution is exposed as
`GET /api/v1/{prefix}/{path:path}`; regular users are authorized by exact
prefix/username matching before the database lookup.

Dynamic SQL is parsed with SQLGlot's selected Trino/PostgreSQL dialect and
must be exactly one
`SELECT` or `WITH ... SELECT`. AST traversal rejects DML, DDL, command,
privilege/session and transaction statements. The canonical SQL generated by
the validator is the only SQL sent to the selected database. Query values use typed
SQLAlchemy bind parameters, so a value such as `APAC' OR 1=1 --` remains a
string value and cannot modify the statement.

Route definitions are persisted in PostgreSQL (`dynamic_routes`) and survive
worker restarts. In addition to SQL and typed parameters, each route stores
`db_type` (`trino`/`postgres`), nullable `pii_type`
(`account_id`/`customer_id`) and `response_type` (`data`/`paginated`). The PII
setting maps the exact configured result column through the existing cache.
Paginated routes accept `page` and `page_size` and return data plus pagination
metadata. There is no in-memory route registry, `lab_test_result`, versioning
or soft delete. DELETE is hard delete.
...
See [docs/dynamic-api.md](docs/dynamic-api.md) for the complete safety
pipeline, request examples, parameter contract and audit policy.
